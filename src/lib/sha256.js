// Pure-JS SHA-256 / HMAC-SHA256 / PBKDF2-SHA256 (FIPS 180-4, RFC 2104, RFC 8018).
// No WebCrypto dependency so it works in plain-HTTP contexts and Node for tests.

const K = new Uint32Array([
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
]);

function rotr(x, n) {
  return (x >>> n) | (x << (32 - n));
}

function toBytes(input) {
  if (input instanceof Uint8Array) return input;
  return new TextEncoder().encode(String(input));
}

function sha256Bytes(bytes) {
  const l = bytes.length;
  const bitLen = l * 8;
  const paddedLen = (((l + 8) >> 6) + 1) << 6;
  const m = new Uint8Array(paddedLen);
  m.set(bytes);
  m[l] = 0x80;
  const view = new DataView(m.buffer);
  view.setUint32(paddedLen - 8, Math.floor(bitLen / 0x100000000), false);
  view.setUint32(paddedLen - 4, bitLen >>> 0, false);

  let h0 = 0x6a09e667, h1 = 0xbb67ae85, h2 = 0x3c6ef372, h3 = 0xa54ff53a;
  let h4 = 0x510e527f, h5 = 0x9b05688c, h6 = 0x1f83d9ab, h7 = 0x5be0cd19;
  const w = new Uint32Array(64);

  for (let i = 0; i < paddedLen; i += 64) {
    for (let j = 0; j < 16; j++) w[j] = view.getUint32(i + j * 4, false);
    for (let j = 16; j < 64; j++) {
      const s0 = rotr(w[j - 15], 7) ^ rotr(w[j - 15], 18) ^ (w[j - 15] >>> 3);
      const s1 = rotr(w[j - 2], 17) ^ rotr(w[j - 2], 19) ^ (w[j - 2] >>> 10);
      w[j] = (w[j - 16] + s0 + w[j - 7] + s1) | 0;
    }
    let a = h0, b = h1, c = h2, d = h3, e = h4, f = h5, g = h6, h = h7;
    for (let j = 0; j < 64; j++) {
      const S1 = rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25);
      const ch = (e & f) ^ (~e & g);
      const temp1 = (h + S1 + ch + K[j] + w[j]) | 0;
      const S0 = rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22);
      const maj = (a & b) ^ (a & c) ^ (b & c);
      const temp2 = (S0 + maj) | 0;
      h = g; g = f; f = e; e = (d + temp1) | 0;
      d = c; c = b; b = a; a = (temp1 + temp2) | 0;
    }
    h0 = (h0 + a) | 0; h1 = (h1 + b) | 0; h2 = (h2 + c) | 0; h3 = (h3 + d) | 0;
    h4 = (h4 + e) | 0; h5 = (h5 + f) | 0; h6 = (h6 + g) | 0; h7 = (h7 + h) | 0;
  }
  return new Uint8Array([
    (h0 >>> 24) & 0xff, (h0 >>> 16) & 0xff, (h0 >>> 8) & 0xff, h0 & 0xff,
    (h1 >>> 24) & 0xff, (h1 >>> 16) & 0xff, (h1 >>> 8) & 0xff, h1 & 0xff,
    (h2 >>> 24) & 0xff, (h2 >>> 16) & 0xff, (h2 >>> 8) & 0xff, h2 & 0xff,
    (h3 >>> 24) & 0xff, (h3 >>> 16) & 0xff, (h3 >>> 8) & 0xff, h3 & 0xff,
    (h4 >>> 24) & 0xff, (h4 >>> 16) & 0xff, (h4 >>> 8) & 0xff, h4 & 0xff,
    (h5 >>> 24) & 0xff, (h5 >>> 16) & 0xff, (h5 >>> 8) & 0xff, h5 & 0xff,
    (h6 >>> 24) & 0xff, (h6 >>> 16) & 0xff, (h6 >>> 8) & 0xff, h6 & 0xff,
    (h7 >>> 24) & 0xff, (h7 >>> 16) & 0xff, (h7 >>> 8) & 0xff, h7 & 0xff,
  ]);
}

function hmacSha256Bytes(key, msg) {
  const keyBytes = toBytes(key);
  const msgBytes = toBytes(msg);
  const block = 64;
  const norm = keyBytes.length > block ? sha256Bytes(keyBytes) : keyBytes;
  const keyPadded = new Uint8Array(block);
  keyPadded.set(norm);
  const iPad = new Uint8Array(block);
  const oPad = new Uint8Array(block);
  for (let i = 0; i < block; i++) {
    iPad[i] = keyPadded[i] ^ 0x36;
    oPad[i] = keyPadded[i] ^ 0x5c;
  }
  const inner = new Uint8Array(block + msgBytes.length);
  inner.set(iPad);
  inner.set(msgBytes, block);
  const innerHash = sha256Bytes(inner);
  const outer = new Uint8Array(block + innerHash.length);
  outer.set(oPad);
  outer.set(innerHash, block);
  return sha256Bytes(outer);
}

export function toHex(bytes) {
  let out = "";
  for (let i = 0; i < bytes.length; i++) out += bytes[i].toString(16).padStart(2, "0");
  return out;
}

export function sha256(input) {
  return toHex(sha256Bytes(toBytes(input)));
}

export function hmacSha256(key, msg) {
  const bytes = hmacSha256Bytes(key, msg);
  return { hex: toHex(bytes), bytes };
}

export function pbkdf2Sha256(password, salt, iterations, dkLen = 32) {
  const p = toBytes(password);
  const s = toBytes(salt);
  const hLen = 32;
  const blocks = Math.ceil(dkLen / hLen);
  const out = new Uint8Array(blocks * hLen);
  const one = new Uint8Array(4);
  for (let bi = 1; bi <= blocks; bi++) {
    one[0] = (bi >>> 24) & 0xff; one[1] = (bi >>> 16) & 0xff; one[2] = (bi >>> 8) & 0xff; one[3] = bi & 0xff;
    const saltBlock = new Uint8Array(s.length + 4);
    saltBlock.set(s);
    saltBlock.set(one, s.length);
    let u = hmacSha256Bytes(p, saltBlock);
    const t = new Uint8Array(u.length);
    t.set(u);
    for (let i = 1; i < iterations; i++) {
      const next = hmacSha256Bytes(p, u);
      for (let j = 0; j < hLen; j++) t[j] ^= next[j];
      u = next;
    }
    out.set(t, (bi - 1) * hLen);
  }
  return toHex(out.subarray(0, dkLen));
}